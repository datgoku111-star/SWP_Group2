  ...
  const OrderCard = ({ order }: { order: ServiceOrder }) => (
    <div className="p-5 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-2xl shadow-sm space-y-4">
      <div className="flex justify-between items-start">
        <div>
          <div className="text-xl font-bold">Room {order.booking?.room?.room_number}</div>
      ...
      <div className="border-t border-b border-neutral-100 dark:border-neutral-800 py-3 space-y-2">
        {order.items?.map((item: any, i: number) => (
          <div key={i} className="flex justify-between items-center text-sm">
            <span className="font-medium">{item.quantity}x {item.service?.name}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-neutral-50 dark:bg-neutral-800/50 p-6 rounded-3xl min-h-[500px]">
          <h3 className="text-xl font-semibold mb-6 flex items-center justify-between">
            ...
            <span className="bg-amber-100 text-amber-800 px-3 py-1 rounded-full text-sm">{pendingOrders.length}</span>
          </h3>
          <div className="space-y-4">
            {pendingOrders.map(order => <OrderCard key={order.id} order={order} />)}
            {pendingOrders.length === 0 && <div className="text-center text-neutral-500 py-10">No pending orders.</div>}
          </div>
        </div>
        ...
        <div className="bg-neutral-50 dark:bg-neutral-800/50 p-6 rounded-3xl min-h-[500px]">
          <h3 className="text-xl font-semibold mb-6 flex items-center justify-between">
            ...
            <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">{inProgressOrders.length}</span>
          </h3>
          <div className="space-y-4">
            {inProgressOrders.map(order => <OrderCard key={order.id} order={order} />)}
            {inProgressOrders.length === 0 && <div className="text-center text-neutral-500 py-10">No orders in progress.</div>}
          </div>
        </div>
      </div>
    </div>
