...
export interface TabFiltersProps {
  ...
  typeOfCarSelected: string[];
  setTypeOfCarSelected: (types: string[]) => void;
}
...
const TabFilters: FC<TabFiltersProps> = ({
  ...
  setRangePrices,
  typeOfCarSelected,
  setTypeOfCarSelected,
}) => {
  const [isOpenMoreFilter, setisOpenMoreFilter] = useState(false);
  const [isOpenMoreFilterMobile, setisOpenMoreFilterMobile] = useState(false);
  const [localRangePrices, setLocalRangePrices] = useState<number[]>(rangePrices);
  const [localTypeOfCar, setLocalTypeOfCar] = useState<string[]>(typeOfCarSelected);
  const [isOnSale, setIsOnSale] = useState(true);

  useEffect(() => {
    setLocalRangePrices(rangePrices);
  }, [rangePrices]);

  useEffect(() => {
    setLocalTypeOfCar(typeOfCarSelected);
  }, [typeOfCarSelected]);
          ...
          <>
            <Popover.Button
              className={`flex items-center justify-center px-4 py-2 text-sm rounded-full border border-neutral-300 dark:border-neutral-700 hover:border-neutral-400 dark:hover:border-neutral-6000 focus:outline-none ${
                typeOfCarSelected.length > 0 ? "border-primary-500 bg-primary-50 text-primary-700" : ""
              }`}
            >
              <span>
                {typeOfCarSelected.length > 0
                  ? `Car type (${typeOfCarSelected.length})`
                  : "Car type"}
              </span>
              ...
              <Popover.Panel className="absolute z-10 w-screen max-w-sm px-4 mt-3 left-0 sm:px-0 lg:max-w-md">
                <div className="overflow-hidden rounded-2xl shadow-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700">
                    ...
                    {typeOfCar.map((item) => (
                      <div key={item.name} className="">
                        <Checkbox
                          ...
                          defaultChecked={localTypeOfCar.includes(item.name)}
                          onChange={(checked) => {
                            if (checked) {
                              setLocalTypeOfCar([...localTypeOfCar, item.name]);
                            } else {
                              setLocalTypeOfCar(localTypeOfCar.filter((t) => t !== item.name));
                            }
                          }}
                        />
                      </div>
                  ...
                  <div className="p-5 bg-neutral-50 dark:bg-neutral-900 dark:border-t dark:border-neutral-800 flex items-center justify-between">
                    ...
                    <ButtonPrimary
                      onClick={() => {
                        setTypeOfCarSelected(localTypeOfCar);
                        close();
                      }}
                      sizeClass="px-4 py-2 sm:px-5"
                    >
  ...
  const renderTabsPriceRage = () => {
    const isPriceActive = rangePrices[0] !== 0 || rangePrices[1] !== 1000;
    return (
      <Popover className="relative">
        {({ open, close }) => (
          <>
              ...
              <Popover.Panel className="absolute z-10 w-screen max-w-sm px-4 mt-3 left-0 sm:px-0 ">
                <div className="overflow-hidden rounded-2xl shadow-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700">
                  ...
                  <div className="p-5 bg-neutral-50 dark:bg-neutral-900 dark:border-t dark:border-neutral-800 flex items-center justify-between">
                    ...
                    <ButtonPrimary
                      onClick={() => {
                        setRangePrices(localRangePrices);
                        close();
                      }}
                      sizeClass="px-4 py-2 sm:px-5"
                    >
        ...
        <div className="flex flex-col space-y-5">
          {list1.map((item) => (
            <Checkbox
              ...
              subLabel={item.description}
              label={item.name}
              defaultChecked={selectedValues ? selectedValues.includes(item.name) : !!item.defaultChecked}
              onChange={(checked) => onChangeValue && onChangeValue(item.name, checked)}
            />
          ))}
        </div>
        <div className="flex flex-col space-y-5">
                    ...
                    <div className="px-4 sm:px-6 divide-y divide-neutral-200 dark:divide-neutral-800">
                      ...
                      <div className="py-7">
                        ...
                        <div className="mt-6 relative ">
                          {renderMoreFilterItem(typeOfCar, localTypeOfCar, (name, checked) => {
                            if (checked) {
                              setLocalTypeOfCar([...localTypeOfCar, name]);
                            } else {
                              setLocalTypeOfCar(localTypeOfCar.filter((t) => t !== name));
                            }
                          })}
                        </div>
                      </div>
