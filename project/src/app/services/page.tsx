...
export default function ServicesPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const cats = Array.from(
          new Set(data.map((s: Service) => s.category))
        );
    ...
    try {
      ...
      if (res.ok) {
        ...
        const myBookings = data.filter(
          (b: Booking) =>
            b.user_id === user?.id && b.status === "CHECKED_IN"
        );
  ...
  const placeOrder = async () => {
    ...
    const items = Object.entries(cart).map(([service_id, quantity]) => ({
      service_id,
      quantity,
    }));
      ...
      <div className="flex justify-between items-center mb-10">
        <div>
          ...
          <p className="text-neutral-500 mt-2">
            Order food, drinks, laundry, and more directly to your room.
          </p>
        </div>
