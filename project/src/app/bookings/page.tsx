...
export default function BookingsPage() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // If customer, filter to only their bookings
        // (Though the API should ideally do this, our simple mock returns all for staff)
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case "CHECKED_OUT": return "bg-neutral-100 text-neutral-800";
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-neutral-500 dark:text-neutral-400">
            ...
            <tbody className="divide-y divide-neutral-200 dark:divide-neutral-700">
              {bookings.map((booking) => (
                <tr key={booking.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="font-medium text-neutral-900 dark:text-white">{booking.room?.room_type?.name}</div>
                    <div>Room {booking.room?.room_number}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center"><Calendar className="w-4 h-4 mr-1" /> {booking.check_in_date}</div>
                    <div className="text-xs text-neutral-400 ml-5">to {booking.check_out_date}</div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link href={`/bookings/${booking.id}` as Route} className="inline-flex items-center text-primary-6000 hover:text-primary-700 font-medium">
              ...
              {bookings.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center">No bookings found.</td>
                </tr>
              )}
            </tbody>
          </table>
