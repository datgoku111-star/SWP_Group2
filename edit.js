...
const file_path = 'project/src/components/ReceptionistServiceHub.tsx';
...
const button_target = '<button onClick={fetchAllData} className="p-3 rounded-2xl bg-white/10 hover:bg-white/20 transition-colors" title="Làm mới">';
const button_replacement = `<button
...
// 3. Add handlers
const handler_target = 'const handleCancelOrder = async (orderId: string) => {';
const handler_replacement = `const handleCheckoutAction = async (bookingId: string, action: string) => {
    try {
...
const tab_content_target = '{/* MODAL ORDER ROOM SERVICE / F&B FOR GUEST */}';
const tab_content_replacement = `{/* SUB-TAB 5: CHECKOUT REQUESTS */}
      {activeSubTab === "CHECKOUTS" && (
