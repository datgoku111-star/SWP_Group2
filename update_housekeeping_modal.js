...
// 2. Replace the button action in the DIRTY flow. Note: We use Regex to avoid whitespace issues.
const oldButtonRegex = /<button[\s\S]*?onClick=\{\(\) => \{[\s\S]*?const reason = prompt[\s\S]*?if \(reason !== null\) changeStatus\(room.id, "MAINTENANCE", reason\);[\s\S]*?\}\}[\s\S]*?Báo Hỏng \/ Bảo trì[\s\S]*?<\/button>/;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-white dark:bg-neutral-900 rounded-3xl p-6 max-w-md w-full shadow-2xl">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="p-3 rounded-2xl border border-neutral-200 dark:border-neutral-700">
